...
export class Polygon {
  ...
  toArray() {
    ...
    return this.points
      .map((p, i) => (i === 0 ? [ov, p.x, p.y, p.z] : [p.x, p.y, p.z]))
      .flat();
  }
  ...
  alignment() {
    ...
    return (this._aligned =
      diff.x === false ? "yz" : diff.y === false ? "xz" : "xy");
  }
        ...
        if (eo++ % 2) {
          ...
          if (d >= min && d <= max) {
            if (lines) {
              ...
              cloud.push(newPoint((lp.x + p.x) / 2, y, z));
            }
          }
        } else {
          lp = p;
        ...
        if (eo++ % 2) {
          ...
          if (d >= min && d <= max) {
            if (lines) {
              ...
              cloud.push(newPoint(x, (lp.y + p.y) / 2, z));
            }
          }
        } else {
          lp = p;
   ...
   */
  detectArcs(opts = {}) {
    const { tolerance = 0.1, arcRes = 0.1, minPoints = 4 } = opts;
    ...
    while (i < length - minPoints + 1) {
      ...
      const arcData = this._detectArcAt(
        i,
        points,
        length,
        tolerance,
  ...
  _detectArcAt(startIdx, points, length, tolerance, arcRes, minPoints) {
    ...
    for (let idx = startIdx; idx < length; idx++) {
      ...
      if (candidates.length >= minPoints) {
        ...
        const arcData = this._validateArc(
          candidates,
          tolerance,
          arcRes,
          minPoints
   ...
   * @param {String[]} fields to copy in individual points beyond class fields
   * @returns {Polygon}
   */
  clone(deep, fields, point_fields) {
    let np = newPolygon().copyZ(this.getZ()),
      ln = this.length,
      i = 0;

    while (i < ln) np.push(this.points[i++], point_fields);
    ...
    if (deep && this.inner) {
      np.inner = this.inner.clone(false, fields, point_fields);
    }
  ...
  avgZ() {
    return (
      [...this.points.map((p) => p.z)].reduce((a, v) => a + v) /
      this.points.length
    );
  }
  ...
  addVerts(verts) {
    for (let i = 0; i < verts.length; ) {
      this.add(verts[i++], verts[i++], verts[i++]);
    }
    return this;
  }
  ...
  applyRotations() {
    for (let point of this.points) {
      if (point.a) {
        let p2 = new Vector3(point.x, point.y, point.z).applyAxisAngle(
          XAXIS,
          point.a * DEG2RAD
        );
        point.x = p2.x;
  ...
  intersects(poly) {
    let p0 = this.points.slice();
    p0.push(p0[0]);
    let p1 = poly.points.slice();
    p1.push(p1[0]);
  ...
  bisect(p1, p2) {
    ...
    return [
      copy.emitSegment(int[0], int[1]),
      copy.emitSegment(int[1], int[0]).reverse(),
    ];
  }
  ...
  hasPointsInside(poly, tolerance) {
    ...
    this.forEachSegment((prev, next) => {
      ...
      if (prev.distTo2D(next) > config.precision_midpoint_check_dist) {
        ...
        if (
          mid.inPolygon(poly) ||
          mid.nearPolygon(poly, tolerance || config.precision_close_to_poly_sq)
        ) {
          return (exit = true);
        }
      }
      if (
        next.inPolygon(poly) ||
        next.nearPolygon(poly, tolerance || config.precision_close_to_poly_sq)
      ) {
        return (exit = true);
      }
    });
  ...
  simple() {
    return this.clean(
      true,
      undefined,
      Math.min(config.clipper / 10, config.clipperClean * 5)
    );
  }
  ...
  isEquivalent(poly, recurse, precision) {
    ...
    if (
      util.isCloseTo(area1, area2, precision || config.precision_poly_area) &&
      ...
      let c1 = this.circularity(),
        c2 = poly.circularity();
      if (
        Math.abs(c1 - c2) < config.precision_circularity &&
        1 - c1 < config.precision_circularity
      ) {
        return true;
      }
      ...
      this.forEachPoint((i2p) => {
        ...
        poly.forEachSegment((i1p1, i1p2) => {
          ...
          if (
            (dist = i2p.distToLine(i1p1, i1p2)) < config.precision_poly_merge
          ) {
            return (pointok = true);
          }
  ...
  diff(poly) {
    let clip = new Clipper(),
      ...
      sp2 = poly.toClipper(),
      fillang =
        this.fillang && this.area() > poly.area() ? this.fillang : poly.fillang;
  ...
  intersect(poly, min) {
    ...
    if (clip.Execute(ClipIntersect, tree, FillNonZero, FillNonZero)) {
      let inter = POLY.fromClipperTreeUnion(tree, poly.getZ(), minarea)
  ...
  union(poly, min, all) {
    ...
    let clip = new Clipper(),
      ...
      sp2 = poly.toClipper(),
      fillang =
        this.fillang && this.area() >= poly.area()
          ? this.fillang
          : poly.fillang;
  ...
  ribbonMesh(swap) {
    ...
    p0 = p0.slice(min.i).concat(p0.slice(0, min.i));
    p0.push(p0[0]);
    p1 = p1.slice(min.j).concat(p1.slice(0, min.j));
    p1.push(p1[0]);
  ...
  extrude(z = 1, opt = {}) {
    ...
    if (z === 0) {
      return earcut
        .map((face) => face.points.map((p) => [p.x, p.y, p.z]))
        .flat()
        .flat();
    }
    ...
    if (chamfer_bottom) {
      ...
      if (inset.length === 1) {
        ...
        let renest = POLY.renest([
          this.clone(true).setZ(z_side_bottom),
          inset[0],
        ]);
        for (let rnpoly of renest) {
